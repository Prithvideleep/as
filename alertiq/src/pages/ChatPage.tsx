...
import {
  ...
  Clock, MessagesSquare, XCircle, MessageCircle,
} from "lucide-react";
    ...
    <div ref={ref} style={{ position: "relative", flex: 1, minWidth: 0 }}>
      <button
        ...
        style={{
          ...
          cursor: "pointer", width: "100%", maxWidth: 560,
...
export default function ChatPage() {
  ...
  // On mount: if NOT a context-transfer navigation, clear messages
  useEffect(() => {
  ...
  // Auto-send pending context from Investigation
  useEffect(() => {
    if (pendingChatContext && !didConsumeContext.current) {
      didConsumeContext.current = true;
      isTransferSession.current = true;
      const ctx = pendingChatContext;
      setPendingChatContext(null);
      setChatIncidentId(selectedIncidentId);
      ...
      setTimeout(() => {
        setMessages((prev) => [...prev, { id: `a-ctx-${Date.now()}`, role: "assistant", content: findResponse(selectedIncidentId, "fix resolve action recommendation") }]);
        setIsTyping(false);
      }, 900);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%", padding: "var(--page-pad)", paddingBottom: 16, overflow: "hidden" }}>
      <div style={{ maxWidth: 860, margin: "0 auto", width: "100%", display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flexShrink: 0, marginBottom: 14 }}>
          ...
          {/* Incident selector */}
          <ChatIncidentSelector value={chatIncidentId} onChange={handleIncidentChange} />
        </div>
